...
<sub>[← p.374](0374.md) · [index](../README.md) · [PDF p.375](https://ch32-riscv-ug.github.io/CH32V20x/datasheet_zh/CH32FV2x_V3xRM.PDF#page=375) · [p.376 →](0376.md)</sub>
...
件操作INRQ位进入初始化模式，随后退出初始化，才能退出离线状态。  

图24-6 CAN错误状态切换图  
![图24-6](../assets/p375-line-00012.png) <!-- rendered from the PDF; verify at https://ch32-riscv-ug.github.io/CH32V20x/datasheet_zh/CH32FV2x_V3xRM.PDF#page=375 -->
...
● 同步段（SS）：也就是CAN标准里的同步段，固定为1个最小时间单元，正常情况下所期望的  
位跳变发生在本时间段内。  
● 时间段1（BS1）：包含CAN标准里的传播时间段和相位缓冲段1，可以被设置为包含1到16最  
小时间单元，可以被自动延长，用于补偿CAN总线上不同节点频率精度误差带来的相位正向漂移。  
该时间段结束为采样点位置。  
● 时间段2（BS2）：也就是CAN标准里的相位缓冲段2，可以被设置为1到8个最小时间单元，  
可以被自动缩短，以补偿CAN总线上不同节点频率精度误差带来的相位负向漂移。  
重新同步跳转宽度（SJW），是每位中可以延长和缩小的最小时间单元数量上限，范围可设置为1  
到4个最小时间单元。  
上述参数都可以在CAN总线时序寄存器CAN_BTIMR里配置。  
<!-- image: p375-draw-image-00001 bbox=[507.6, 786.0, 527.52, 797.04] -->
<!-- footer: V2.5 372 -->
