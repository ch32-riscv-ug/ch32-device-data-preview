...
<sub>[← p.215](0215.md) · [index](../README.md) · [PDF p.216](https://ch32-riscv-ug.github.io/CH32X315/datasheet_zh/CH32X315RM.PDF#page=216) · [p.217 →](0217.md)</sub>

<!-- header: CH32X315、X305应用手册 https://wch.cn -->
据；若后续准备进入主设备接收模式，则需要重新发送一个起始条件，跟随发送一个字节为  
11110xx1，然后进入主设备接收模式。  

图16-3 10位地址时主机收发数据示意图  
![图16-3](../assets/p216-line-00004.png) <!-- rendered from the PDF; verify at https://ch32-riscv-ug.github.io/CH32X315/datasheet_zh/CH32X315RM.PDF#page=216 -->
...
位。在清除BTF前，SCL将保持低电平，读取状态寄存器1（R16_I2Cx_STAR1）后，再向数据寄存器  
写入数据将会清除BTF位。  
接收模式时，在ADDR被清除后，I2C模块将SDA上的数据通过移位寄存器存进数据寄存器，在  
