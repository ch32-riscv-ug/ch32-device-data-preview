...
<sub>[← p.96](0096.md) · [index](../README.md) · [PDF p.97](https://ch32-riscv-ug.github.io/CH32M030/datasheet_zh/CH32M030RM.PDF#page=97) · [p.98 →](0098.md)</sub>
...
生捕获事件时，CCxIF（在R16_TIM1_INTFR中）被置位，如果使能了中断或者DMA，还会产生相应中  
断或者DMA。如果发生捕获事件时，CCxIF已经被置位了，那么CCxOF位会被置位。CCxIF可由软件清  
除，也可以通过读取比较捕获寄存器由硬件清除。CCxOF由软件清除。  
举个通道1的例子来说明使用输入捕获模式的步骤，如下：  
1）配置CCxS域，选择ICx信号的来源。比如设为10b，选择TI1FP1作为IC1的来源，而不可以使用  
默认设置，CCxS域默认是使比较捕获模块作为输出通道；  
2）配置 ICxF 域，设定 TI 信号的数字滤波器。数字滤波器会以确定的频率，采样确定的次数，再输  
...
较捕获寄存器的值一致时，输出特定的变化或波形。OCxM 域（在 R16_TIM1_CHCTLRx 中）和 CCxP 位  
（在 R16_TIM1_CCER 中）决定输出的是确定的高低电平还是电平翻转。产生比较一致事件时还会置  
