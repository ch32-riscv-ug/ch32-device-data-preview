...
<sub>[← p.122](0122.md) · [index](../README.md) · [PDF p.123](https://ch32-riscv-ug.github.io/CH32M030/datasheet_zh/CH32M030RM.PDF#page=123) · [p.124 →](0124.md)</sub>
...
### 10.2.2 通用定时器和高级定时器的区别

与高级定时器相比，通用定时器缺少以下功能：  
1） 通用定时器缺少对核心计数器的计数周期进行计数的重复计数寄存器。  
2） 通用定时器的比较捕获部分通道拥有简单的死区产生功能，互补输出通过复用其余通道输出。  
3） 通用定时器没有刹车信号机制。  
...
图10-2 通用定时器CK_PSC来源框图  
![图10-2](../assets/p123-line-00012.png) <!-- rendered from the PDF; verify at https://ch32-riscv-ug.github.io/CH32M030/datasheet_zh/CH32M030RM.PDF#page=123 -->
...
2) 内部HB时钟输入路线：CK_INT；  
3) 来自比较捕获通道引脚（TIM2_CHx）的路线：TIM2_CHx→TIx→TIxFPx，此路线也用于编码器模  
式；  
4) 来自内部其他定时器的输入：ITRx。  
